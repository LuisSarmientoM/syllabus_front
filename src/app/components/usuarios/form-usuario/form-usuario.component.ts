...
export class FormUsuarioComponent implements OnInit, OnDestroy {
 ...
 getUsuario() {
  this.form.setValue({
   ...
   primerApellido: this.router.snapshot.queryParamMap.get('primerApellido'),
   segundoApellido: this.router.snapshot.queryParamMap.get('segundoApellido'),
   numeroDocumento: this.router.snapshot.queryParamMap.get('numeroDocumento'),
 ...
 guardar() {
  ...
  if (this.form.valid) {
   if (this.id) {
    ...
    this.sub = this._usuarioService.crearUsuario(this.form.value).subscribe();
   }
  }
 }
