...
export class FormMateriaComponent implements OnInit {
    ...
    private async buildForm() {
        ...
        if (this.id !== "nuevo") {
            this._materiaService.obtenerDataMateria(this.id, this.seccion).subscribe((res: any) => {
                ...
                } else if (this.id.split("-")[1] === "nuevo" && this.seccion === "criterio") {
                    res = {
                        tipo: [
                            {
                                detalleTipo: [],
    ...
    addControl(campo, index) {
        ...
        if (this.seccion === "criterio") {
            ...
            const control: any = this.ns.getCampo(this.seccion, campo.controls.length | 0, campo.label, "textbox");

            this.getForm(campo.key).push(this.formBuilder.control(control.defaultValue, control.required));
    ...
    eliminar(campoKey, index: number, indexTipo?): void {
        if (this.seccion === "criterio") {
            let tipo = this.getForm(campoKey).controls[indexTipo].get("detalleTipo") as FormArray;
            tipo.removeAt(index);
    ...
    guardar() {
        if (this.form.invalid) console.log(this.form.controls);
        if (this.form.valid) {
            if (this.id === "nuevo") {
                ...
                this._materiaService.actualizarMateria(this.id, this.form.value, this.seccion);
            }
        }
    }
